   ...
   /**
    ...
    public static class TrieAutocomplete implements Autocompletor {
        ...
        private void add(String word, double weight) {
            ...
            if (weight < 0) throw new IllegalArgumentException("we dont wanna weight be negative");
            //set weight to the root, if weight > curr weight
            if(myRoot.mySubtreeMaxWeight < weight) {
               myRoot.mySubtreeMaxWeight = weight;
            }
            //updata weight of root 
            Node curr = myRoot;
            for (int i = 0; i < word.length(); i++){
               //change  weight for each chil nodes in for loop
               if(curr.mySubtreeMaxWeight < weight) {
                  curr.mySubtreeMaxWeight = weight; 
               }
               //once we change the weight of current node
               // we need check wether curr exits
               //if curr doesnt exits chil node with next char, we need create new one.
               if(!curr.children.containsKey(word.charAt(i))){
                  curr.children.put(word.charAt(i), new Node(word.charAt(i),curr, weight));
                  curr.myInfo = curr.myInfo + word.charAt(i);
               }
               // move to next node
            ...
            // change, add for loop above
            /*if(curr.mySubtreeMaxWeight < weight) {
               curr.mySubtreeMaxWeight = weight; 
            }*/
            // set value of isWord, myWord and setWord for all word nodes
            curr.isWord = true;
            curr.myWord = word;
            curr.setWord(word);
            //this curr is very last one letter
            
            if(curr.isWord && curr.myWeight > weight){
            //change curr weight to given weight from last one to the first one
   			curr.myWeight = weight;
            curr.setWeight(weight);
            while(curr != null) { 
               double maxWeight = -1;
   				for(Character chr : curr.children.keySet()){
   				   if(curr.children.get(chr).mySubtreeMaxWeight > maxWeight){
   						   maxWeight = curr.children.get(chr).mySubtreeMaxWeight;
   					}
   				}
        ...
        public Iterable<String> topMatches(String prefix, int k) {
            ...
            //as descreption, we need build priorityQueue to hold the first Kth topmatches
            //most diffcult part
            PriorityQueue<Node> pQueue = new PriorityQueue<Node>(k, new Node.ReverseSubtreeMaxWeightComparator());
            ...
            while(!pQueue.isEmpty()) {
               ...
               for (Character chr : curr.children.keySet()){
                  pQueue.add(curr.children.get(chr));
               }
            }
            ...
            int lengthOfList = nodes.length - 1;
            for (int i = 0; i < nodes.length && i < k; i++){
               int index = lengthOfList - i;
               aList.add(nodes[index].getWord());
            }
            return aList;
        }
            ...
            for(int i = 0; i < prefix.length();i++) { 
               if(curr.children.containsKey(prefix.charAt(i))){
                 curr = curr.children.get(prefix.charAt(i));
                  ...
                  return "";// if no, return noting
               }
            }
            if(curr.mySubtreeMaxWeight == curr.getWeight() && curr.isWord) {
               return curr.getWord();
            }
          //if a given string does not match in pre-tire, create new branch and set weight  
